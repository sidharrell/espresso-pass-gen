	...
	*/	
	...
	function espresso_generate_pass_shortcode($atts) {
		...
		if (!empty($_REQUEST['_wpnonce']) 
						&& wp_verify_nonce( $_REQUEST['_wpnonce'], $this->_nonce_field )
						&& !empty($_SESSION[$this->_nonce_field])) {
			for ( $i = 0; $i < $number; $i++ ) {
				$groupon_code = $groupon_codes[$i] = uniqid();
				$wpdb->insert( EVENTS_GROUPON_CODES_TABLE, 
								array ('groupon_code' => $groupon_code,
										'groupon_holder' => $this->_groupon_holder,
										'event_id' => $_REQUEST['pass_events'][$i]), 
								array ('%s',
										'%s'));
			}
